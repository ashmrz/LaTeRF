...
def render_path(render_poses, hwf, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0,
                ...
                post_opening=False, ret_acc=False):
    H, W, focal = hwf
    if render_factor != 0:
...
def render_rays(ray_batch,
    ...
    if network_fn is not None:
        ...
        rgb_map, disp_map, acc_map, weights, depth_map, prob_map = raw2outputs(raw, z_vals, rays_d, raw_noise_std,
                                                                               white_bkgd,
                                                                               pytest=pytest,
                                                                               only_object=only_object,
                                                                               threshold=threshold,
    ...
    if N_importance > 0:
        rgb_map_0, disp_map_0, acc_map_0, prob_map_0 = rgb_map, disp_map, acc_map, prob_map
    ...
    ret = {'rgb_map': rgb_map, 'disp_map': disp_map, 'acc_map': acc_map, 'depth_map': depth_map,
           'weights': weights, 'z_vals': z_vals, 'prob_map': prob_map}
    if retraw:
        ret['raw'] = raw
    if N_importance > 0:
        ret['rgb0'] = rgb_map_0
...
def config_parser():
    ...
    parser.add_argument("--text_query", type=str, default='A red cube',
                        help='A phrase or sentence describing the object of interest')
    ...
    parser.add_argument("--clip_weight", type=float, default=0.01, help='The weight of the classification loss')
    parser.add_argument("--revert_mask", action='store_true',
                        help="revert the gt object masks to extract everything but the object")

    parser.add_argument("--i_clip", type=int, default=10, help='frequency of calculating the clip loss')
    parser.add_argument("--mask_filter", action='store_true', help="use object mask to filter clutter")
...
def gui_application(args, render_kwargs_test):
    ...
    def set_values():
        args.clf_weight = float(clf.get())
        args.clip_weight = float(clip.get())
        args.i_clip = float(i_clip.get())
        args.clf_reg_weight = float(clf_reg.get())
        args.text_query = clip_text.get()
        args.i_video = int(i_video.get())
        args.render_factor = int(render_factor.get())

    tk.Label(root, text="Clf weight").pack()
    clf = tk.Entry(root, textvariable=tk.StringVar(root, value=str(args.clf_weight)))
    clf.pack()
    tk.Label(root, text="Clf regularization weight").pack()
    clf_reg = tk.Entry(root, textvariable=tk.StringVar(root, value=str(args.clf_reg_weight)))
    clf_reg.pack()
    tk.Label(root, text="CLIP weight").pack()
    clip = tk.Entry(root, textvariable=tk.StringVar(root, value=str(args.clip_weight)))
    clip.pack()
    tk.Label(root, text="i_CLIP").pack()
    i_clip = tk.Entry(root, textvariable=tk.StringVar(root, value=str(args.i_clip)))
    i_clip.pack()
    tk.Label(root, text="CLIP text").pack()
    clip_text = tk.Entry(root, textvariable=tk.StringVar(root, value=str(args.text_query)))
    clip_text.pack()
    tk.Label(root, text="i_video").pack()
    i_video = tk.Entry(root, textvariable=tk.StringVar(root, value=str(args.i_video)))
    i_video.pack()
    tk.Label(root, text="render factor").pack()
    render_factor = tk.Entry(root, textvariable=tk.StringVar(root, value=str(args.render_factor)))
    render_factor.pack()
...
def train():
    ...
    gui = threading.Thread(target=gui_application, args=(args, render_kwargs_test,))
    gui.start()
    ...
    if use_batching:
        ...
        raysDepth_iter = iter(DataLoader(RayDataset(rays_depth), batch_size=N_rand, shuffle=False,
    ...
    for i in trange(start, N_iters):
        ...
        if use_batching:
            ...
            try:
                ...
                raysRGBCLF_iter = iter(
                    DataLoader(RayDataset(rays_rgb_clf), batch_size=N_rand, shuffle=False, num_workers=0))
                batch_clf = next(raysRGBCLF_iter).to(device)
            ...
            if args.colmap_depth:
                ...
                try:
                    ...
                    raysDepth_iter = iter(
                        DataLoader(RayDataset(rays_depth), batch_size=N_rand, shuffle=False, num_workers=0))
                    batch_depth = next(raysDepth_iter).to(device)
        ...
        #######################
        if args.clf_reg_weight > 0:
            rgb_obj, disp_obj, acc_obj, depth_obj, prob_obj, extras_obj = render(H, W, focal, chunk=args.chunk,
                                                                                 rays=batch_rays_clf,
                                                                                 verbose=i < 10, retraw=True,
                                                                                 only_object=True,
                                                                                 **render_kwargs_train)
            valid_indices = torch.logical_not(
                torch.logical_and(
                    torch.logical_and(label_s == 0, depth_clf < depth_obj),
                    acc_obj > 0.5
                )
            )
            loss += nn.MSELoss()(acc_obj[valid_indices], label_s[valid_indices].float()) * args.clf_reg_weight

        ###########
        # CLIP loss
        ###########
        if i % args.i_clip == 0 and i > 0:
            # perm = torch.randperm(render_poses.size(0) + poses.size(0))
            # idx = perm[:1]
            # if idx[0] < render_poses.size(0):
            #     random_poses = render_poses[idx]
            # else:
            #     idx[0] -= render_poses.size(0)
            #     random_poses = poses[idx]
            perm = torch.randperm(poses.size(0))
            idx = perm[:1]
            random_poses = poses[idx]

            if args.clip_weight > 0:
                rgbs, disps, probs, accs = render_path(random_poses, hwf, args.chunk,
                                                       render_kwargs_test,
                ...
                with torch.no_grad():
                    rgbs, disps, probs = render_path(random_poses, hwf,
                                                     args.chunk,
                                                     render_kwargs_test,
                                                     render_factor=args.render_factor,
                                                     only_object=True,
            ...
            # plt.subplot(122)
            # plt.imshow(probs[0].detach().cpu().numpy())
            # plt.colorbar()

            plt.gcf().set_dpi(600)
            os.makedirs('test_renders', exist_ok=True)
            plt.savefig('test_renders/{}_clipweight_{}_clf_weight_{}_rendered.eps'.format(
                expname, args.clip_weight, args.clf_weight
                # , random.randint(0, 100000000000)
            ), format='eps')
            plt.clf()

            if args.clip_weight > 0:
                clip_loss = -img_txt_similarity(rgbs[0], args.text_query)
                loss += clip_loss * args.clip_weight

                if args.dataset_type == 'blender':
                    alpha = alphas_gt[idx[0]][::args.render_factor, ::args.render_factor]
                    loss += torch.mean(accs[0][alpha == 0]) * 0.1

                if i % 100 == 0:
                    tqdm.write("RGB Mean {}\tLoss {}".format(rgbs[0].mean(), loss.item()))
        ...
        if args.i_video > 0 and i % args.i_video == 0 and i > 0:
            ...
            with torch.no_grad():
                rgbs, disps, _ = render_path(render_poses, hwf, args.chunk, render_kwargs_test, only_object=True,
                                             mask_filter=args.mask_filter, render_mask=args.render_mask)
