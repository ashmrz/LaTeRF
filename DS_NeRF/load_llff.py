...
def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True):
    ...
    mskgtdir = os.path.join(basedir, 'images' + sfx + '/label_full')
    if not os.path.exists(imgdir):
    ...
    mskfiles = [os.path.join(mskdir, f.split('.')[0] + '.png') for f in sorted(os.listdir(imgdir)) if
                f.endswith('JPG') or f.endswith('jpg') or f.endswith('jpeg') or f.endswith('png')]
    mskgtfiles = [os.path.join(mskgtdir, f.split('.')[0] + '.png') for f in sorted(os.listdir(imgdir)) if
                f.endswith('JPG') or f.endswith('jpg') or f.endswith('jpeg') or f.endswith('png')]
    ...
    for f in mskgtfiles:
        try:
            ...
            masks_gt.append(-np.ones((imgs.shape[0], imgs.shape[1])))

    masks = np.stack(masks, -1)
    masks = masks / np.max(masks)

    masks_gt = np.stack(masks_gt, -1)
    masks_gt = masks_gt / np.max(masks_gt)

    print('Loaded image data', imgs.shape, poses[:, -1, 0], masks.shape)
    return poses, bds, imgs, masks, masks_gt, mask_indices
...
def load_llff_data(basedir, factor=8, recenter=True, bd_factor=.75, spherify=False, path_zflat=False,
                   spherify_hack=True):
    poses, bds, imgs, masks, masks_gt, mask_indices = _load_data(basedir, factor=factor)  # factor=8 downsamples original imgs by 8x
    ...
    masks_gt = np.moveaxis(masks_gt, -1, 0).squeeze().astype(np.float32)
    images = imgs
    ...
    if spherify:
        ...
        poses_spherify, render_poses_spherify, bds_spherify, sc, T = spherify_poses(poses, bds)
    ...
    masks_gt = (masks_gt).astype(np.float32)

    if masks.shape[-1] == 3:
        masks = masks[:, :, :, 0].squeeze()

    if masks_gt.shape[-1] == 3:
        masks_gt = masks_gt[:, :, :, 0].squeeze()

    return images, poses, bds, render_poses, i_test, masks, masks_gt, mask_indices
...
def load_colmap_depth(basedir, factor=8, bd_factor=.75):
    ...
    _, bds_raw, _, _, _, _ = _load_data(basedir, factor=factor)  # factor=8 downsamples original imgs by 8x
