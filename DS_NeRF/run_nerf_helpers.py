...
class NeRF(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, input_ch_views=3, output_ch=4, skips=[4], use_viewdirs=False):
        ...
        if use_viewdirs:
            ...
            # For the logit output concerning the probability of the object being in a point of the space
            self.object_hidden1 = nn.Linear(input_ch + W, W)
            self.object_hidden2 = nn.Linear(W, W)
            self.object_linear = nn.Linear(W, 1)
            self.rgb_linear = nn.Linear(W // 2, 3)
        else:
            self.output_linear = nn.Linear(W, output_ch + 1)  # the last entry is for the object logit
    ...
    def forward(self, x):
        ...
        if self.use_viewdirs:
            ...
            logit = self.object_hidden1(torch.cat([input_pts, h], -1))
            logit = F.relu(logit)
            logit = self.object_hidden2(logit)
            logit = F.relu(logit)
            logit = self.object_linear(logit)
            ...
            outputs = torch.cat([rgb, alpha, logit], -1)
        else:
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False, only_object=False, threshold=None,
    ...
    if only_object:
        if threshold is None:
            alpha = raw2alpha(raw[..., 3] + noise, dists) * torch.sigmoid(logits)  # [N_rays, N_samples]
        else:
            alpha = raw2alpha(raw[..., 3] + noise, dists) * torch.sigmoid(logits)  # [N_rays, N_samples]
    ...
    prob_map = torch.sum(weights.detach() * logits, -1)
    ...
    return rgb_map, disp_map, acc_map, weights, depth_map, prob_map
